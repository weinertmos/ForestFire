...
See :ref:`decisionnode <decisionnode>`
...
* By which feature (=column) should the next decision be made?
    If a feature is chosen at the first node it should be the one feature that separates the data set in the best possible way
...
loops through the following steps:
    ...
    #. If no split is better than the current list no split is performed and results are stored
