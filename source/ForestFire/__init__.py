...
a machine learning algorithm has a high computational cost. 
...
The machine learning algorithm and the way the raw data is split are configured in two seperate files:

* **import_data.py** - X and y are loaded from the numpy files in the same folder. 
  Apply data splitting methods here and return the train and test data sets.
* **compute.py** - Set up the machine learning algorithm that you want to supply with promising selections of 
  feature sets generated by *ForestFire*.

After *ForestFire* is supplied with the raw Data (X.npy,y.npy), the way this data should be split (import_data.py)
and the designated machine learning algorithm (compute.py) the default setup is complete. 
...
original machine learning algorithm and added to the database. 
